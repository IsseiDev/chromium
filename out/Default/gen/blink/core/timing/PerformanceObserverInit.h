// Copyright 2014 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// This file has been auto-generated by code_generator_v8.py. DO NOT MODIFY!

#ifndef PerformanceObserverInit_h
#define PerformanceObserverInit_h

#include "bindings/core/v8/IDLDictionaryBase.h"
#include "bindings/core/v8/Nullable.h"
#include "core/CoreExport.h"
#include "platform/heap/Handle.h"
#include "wtf/Vector.h"
#include "wtf/text/WTFString.h"

namespace blink {

class CORE_EXPORT PerformanceObserverInit : public IDLDictionaryBase {
    DISALLOW_NEW_EXCEPT_PLACEMENT_NEW();
public:
    PerformanceObserverInit();
    virtual ~PerformanceObserverInit();

    bool hasEntryTypes() const { return !m_entryTypes.isNull(); }
    const Vector<String>& entryTypes() const { return m_entryTypes.get(); }
    void setEntryTypes(const Vector<String>& value) { m_entryTypes = value; }

    v8::Local<v8::Value> toV8Impl(v8::Local<v8::Object>, v8::Isolate*) const override;
    DECLARE_VIRTUAL_TRACE();

private:
    Nullable<Vector<String>> m_entryTypes;

    friend class V8PerformanceObserverInit;
};

} // namespace blink

#endif // PerformanceObserverInit_h
