...
InstrumentingAgents::InstrumentingAgents()
    ...
    , m_hasInspectorWebPerfAgents(false)
...
void InstrumentingAgents::addInspectorWebPerfAgent(InspectorWebPerfAgent* agent)
{
    m_inspectorWebPerfAgents.add(agent);
    m_hasInspectorWebPerfAgents = true;
}

void InstrumentingAgents::removeInspectorWebPerfAgent(InspectorWebPerfAgent* agent)
{
    m_inspectorWebPerfAgents.remove(agent);
    m_hasInspectorWebPerfAgents = !m_inspectorWebPerfAgents.isEmpty();
}
...
DEFINE_TRACE(InstrumentingAgents)
{
    ...
    visitor->trace(m_inspectorWebPerfAgents);
...
void didReceiveData(LocalFrame* paramLocalFrame, unsigned long identifier, const char* data, int dataLength, int encodedDataLength)
{
    InstrumentingAgents* agents = instrumentingAgentsFor(paramLocalFrame);
    if (!agents)
        return;
...
void willExecuteScript(ExecutionContext* paramExecutionContext)
{
    InstrumentingAgents* agents = instrumentingAgentsFor(paramExecutionContext);
    if (!agents)
        return;
    if (agents->hasInspectorWebPerfAgents()) {
        for (InspectorWebPerfAgent* agent : agents->inspectorWebPerfAgents())
            agent->willExecuteScript(paramExecutionContext);
    }
}

void didExecuteScript(ExecutionContext* paramExecutionContext)
{
    InstrumentingAgents* agents = instrumentingAgentsFor(paramExecutionContext);
    if (!agents)
        return;
