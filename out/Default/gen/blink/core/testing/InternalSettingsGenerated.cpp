...
InternalSettingsGenerated::InternalSettingsGenerated(Page* page)
    ...
    , m_allowGeolocationOnInsecureOrigins(page->settings().allowGeolocationOnInsecureOrigins())
    ...
    , m_alwaysShowContextMenuOnTouch(page->settings().alwaysShowContextMenuOnTouch())
    ...
    , m_autoplayExperimentMode(page->settings().autoplayExperimentMode())
    ...
    , m_blockedMainThreadEventsWarningThreshold(page->settings().blockedMainThreadEventsWarningThreshold())
    , m_browserSideNavigationEnabled(page->settings().browserSideNavigationEnabled())
    , m_caretBrowsingEnabled(page->settings().caretBrowsingEnabled())
    , m_cookieEnabled(page->settings().cookieEnabled())
    , m_dataSaverEnabled(page->settings().dataSaverEnabled())
    ...
    , m_disallowFetchForDocWrittenScriptsInMainFrame(page->settings().disallowFetchForDocWrittenScriptsInMainFrame())
    , m_disallowFetchForDocWrittenScriptsInMainFrameOnSlowConnections(page->settings().disallowFetchForDocWrittenScriptsInMainFrameOnSlowConnections())
    ...
    , m_forceMainWorldInitialization(page->settings().forceMainWorldInitialization())
    ...
    , m_inertVisualViewport(page->settings().inertVisualViewport())
    ...
    , m_parseHTMLOnMainThreadCoalesceChunks(page->settings().parseHTMLOnMainThreadCoalesceChunks())
    , m_parseHTMLOnMainThreadSyncTokenize(page->settings().parseHTMLOnMainThreadSyncTokenize())
    ...
    , m_preferHiddenVolumeControls(page->settings().preferHiddenVolumeControls())
    , m_presentationRequiresUserGesture(page->settings().presentationRequiresUserGesture())
    , m_reportScreenSizeInPhysicalPixelsQuirk(page->settings().reportScreenSizeInPhysicalPixelsQuirk())
    , m_resizeIsDeviceSizeChange(page->settings().resizeIsDeviceSizeChange())
    ...
    , m_shouldReuseGlobalForUnownedMainFrame(page->settings().shouldReuseGlobalForUnownedMainFrame())
    ...
    , m_strictMixedContentCheckingForPlugin(page->settings().strictMixedContentCheckingForPlugin())
    , m_strictPowerfulFeatureRestrictions(page->settings().strictPowerfulFeatureRestrictions())
    , m_strictlyBlockBlockableMixedContent(page->settings().strictlyBlockBlockableMixedContent())
    ...
    , m_textTrackMarginPercentage(page->settings().textTrackMarginPercentage())
    ...
    , m_useDefaultImageInterpolationQuality(page->settings().useDefaultImageInterpolationQuality())
...
void InternalSettingsGenerated::resetToConsistentState()
{
    ...
    m_page->settings().setAutoplayExperimentMode(m_autoplayExperimentMode);
    ...
    m_page->settings().setBlockedMainThreadEventsWarningThreshold(m_blockedMainThreadEventsWarningThreshold);
    ...
    m_page->settings().setResizeIsDeviceSizeChange(m_resizeIsDeviceSizeChange);
...
void InternalSettingsGenerated::setAutoplayExperimentMode(const String& autoplayExperimentMode)
{
    m_page->settings().setAutoplayExperimentMode(autoplayExperimentMode);
}
...
void InternalSettingsGenerated::setBlockedMainThreadEventsWarningThreshold(double blockedMainThreadEventsWarningThreshold)
{
    m_page->settings().setBlockedMainThreadEventsWarningThreshold(blockedMainThreadEventsWarningThreshold);
}
...
void InternalSettingsGenerated::setResizeIsDeviceSizeChange(bool resizeIsDeviceSizeChange)
{
    m_page->settings().setResizeIsDeviceSizeChange(resizeIsDeviceSizeChange);
}
