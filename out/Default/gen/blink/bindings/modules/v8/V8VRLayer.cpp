...
void V8VRLayer::toImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8Value, VRLayer& impl, ExceptionState& exceptionState)
{
    ...
    if (leftBoundsValue.IsEmpty() || leftBoundsValue->IsUndefined()) {
        // Do nothing.
    } else if (leftBoundsValue->IsNull()) {
        impl.setLeftBoundsToNull();
    } else {
    ...
    if (rightBoundsValue.IsEmpty() || rightBoundsValue->IsUndefined()) {
        // Do nothing.
    } else if (rightBoundsValue->IsNull()) {
        impl.setRightBoundsToNull();
    } else {
...
bool toV8VRLayer(const VRLayer& impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate)
{
    if (impl.hasLeftBounds()) {
        ...
        if (!v8CallBoolean(dictionary->CreateDataProperty(isolate->GetCurrentContext(), v8String(isolate, "leftBounds"), v8::Null(isolate))))
            return false;
    }
    ...
    if (impl.hasRightBounds()) {
        ...
        if (!v8CallBoolean(dictionary->CreateDataProperty(isolate->GetCurrentContext(), v8String(isolate, "rightBounds"), v8::Null(isolate))))
            return false;
    }
