...
void ChooserServiceProxy::GetPermission(
    std::vector<::device::usb::DeviceFilterPtr> in_device_filters, const GetPermissionCallback& callback) {
...
class ChooserService_GetPermission_ProxyToResponder {
 ...
 private:
  ...
  void Run(
      ::device::usb::DeviceInfoPtr in_result);
...
bool ChooserServiceStub::AcceptWithResponder(
  ...
  switch (message->header()->name) {
    case internal::kChooserService_GetPermission_Name: {
      ...
      (&serialization_context_)->handles.Swap((message)->mutable_handles());
      bool success = true;
      std::vector<::device::usb::DeviceFilterPtr> p_device_filters{};
