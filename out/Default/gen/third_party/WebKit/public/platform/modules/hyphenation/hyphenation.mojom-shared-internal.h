...
#pragma pack(push, 1)
const uint32_t kHyphenation_OpenDictionary_Name = 0;
class Hyphenation_OpenDictionary_Params_Data {
 ...
 private:
  Hyphenation_OpenDictionary_Params_Data() : header_({sizeof(*this), 0}) {
  }
...
class Hyphenation_OpenDictionary_ResponseParams_Data {
 public:
  ...
  mojo::internal::StructHeader header_;
  mojo::internal::Handle_Data hyphenation_dictionary_handle;
  uint8_t padfinal_[4];

 private:
  Hyphenation_OpenDictionary_ResponseParams_Data() : header_({sizeof(*this), 0}) {
  }
