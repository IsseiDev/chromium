...
class CONTENT_EXPORT Hyphenation {
 public:
  ...
  virtual bool OpenDictionary(const mojo::String& locale, mojo::ScopedHandle* hyphenation_dictionary_handle);

  using OpenDictionaryCallback = base::Callback<void(mojo::ScopedHandle)>;
  virtual void OpenDictionary(const mojo::String& locale, const OpenDictionaryCallback& callback) = 0;
};
...
class CONTENT_EXPORT HyphenationProxy
 ...
 public:
  ...
  bool OpenDictionary(const mojo::String& locale, mojo::ScopedHandle* hyphenation_dictionary_handle) override;
  void OpenDictionary(const mojo::String& locale, const OpenDictionaryCallback& callback) override;
...
class CONTENT_EXPORT HyphenationStub : public NON_EXPORTED_BASE(mojo::MessageReceiverWithResponderStatus) {
 public:
  HyphenationStub();
...
class CONTENT_EXPORT HyphenationRequestValidator : public NON_EXPORTED_BASE(mojo::MessageFilter) {
 public:
  explicit HyphenationRequestValidator(mojo::MessageReceiver* sink = nullptr);

  bool Accept(mojo::Message* message) override;
};
class CONTENT_EXPORT HyphenationResponseValidator : public NON_EXPORTED_BASE(mojo::MessageFilter) {
 public:
