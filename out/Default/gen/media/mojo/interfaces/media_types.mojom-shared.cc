...
bool EncryptionScheme_Data::Validate(
  ...
  if (!::media::mojom::internal::CipherMode_Data
...
bool VideoDecoderConfig_Data::Validate(
  ...
  if (!::media::mojom::internal::VideoFormat_Data
        ::Validate(object->format, validation_context))
    return false;


  if (!::media::mojom::internal::ColorSpace_Data
...
bool SubsampleEntry_Data::Validate(
  ...
  // NOTE: The memory backing |object| may be smaller than |sizeof(*object)| if
  // the message comes from an older version.
  const SubsampleEntry_Data* object = static_cast<const SubsampleEntry_Data*>(data);
